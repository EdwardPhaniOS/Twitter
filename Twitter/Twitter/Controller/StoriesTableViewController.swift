...
class StoriesTableViewController: UITableViewController
{
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        // (2) automaticallt download data from the reference every time
        storiesRef.observe(.value) { (snapshot) in
            self.stories.removeAll()
            
            for child in snapshot.children {
                let story = Story(snapShot: child as! DataSnapshot)
    ...
    override func viewDidLoad() {
        ...
        self.tableView.rowHeight = UITableView.automaticDimension + 80
    ...
    {
        self.performSegue(withIdentifier: "Compose Story", sender: nil)
    }
